...
Both documents are written for the LLM agents that consume them downstream (`sdd-create-techspec`, `sdd-create-tasks`, review rounds). Their job is to supply business rules, domain behavior, and product intent. KPIs, success metrics, timelines, and rollout phases have no consumer in this pipeline — leave them out.
...
Capture the complete scope the user wants in this single PRD, however large it grows. Agents run long and `sdd-create-tasks` decomposes the work later, so document size is never a reason to trim, defer, or stage anything.
...
- Multiple-choice whenever the options can be predetermined: labeled options (A, B, C) with your recommendation first, plus a fallback ("D) Other — describe").
...
- Optional: existing `_idea.md` file as primary input for context (see `sdd-idea`).
...
7. Hand off.
   ...
   - Point to `sdd-create-techspec` as the next step.
