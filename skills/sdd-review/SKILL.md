---
name: sdd-review
description: Performs a comprehensive code review of a PRD implementation and generates a review round directory with issue files compatible with sdd-fix-reviews. Use when reviewing implemented PRD tasks or performing a quality audit of code changes. Do not use for fixing existing review issues, executing PRD tasks, or editing source code.
---

# Review Round

Perform a structured code review of a PRD implementation and produce a review round directory that the `sdd-fix-reviews` workflow can process.
...
3. Perform the code review.
   ...
   - Skip issues that linters or formatters already catch. Run the project's lint command first (e.g. `make lint`) to filter these out.
...
4. Generate issue files.
   ...
   - The `pr` field is empty unless the user provides a PR number.
...
5. Summarize and present the review.
   ...
   - Suggest running `sdd-fix-reviews` to process the review round.

6. Verify before completion.
   - Use installed `sdd-final-verify` before claiming the review round is complete.
...
- Do not fix the issues found. This skill only identifies and documents issues. The `sdd-fix-reviews` workflow handles remediation.
- Do not create issue files for problems that linters or formatters already catch.
- Every issue file must have valid YAML frontmatter matching `references/issue-template.md`.
...
- If the lint command fails to run (build errors, missing tools), note the failure in the summary and proceed with the review. Do not skip the review because linting failed — just acknowledge that linter-overlap filtering could not be applied.
