...
Use this skill when asked to resolve conflicts in an integration worktree
(e.g. while `scripts/run-tasks.sh` rebases a task branch onto the integration
branch). The goal is a clean merge result, not a clever shortcut.
...
5. Do not commit. The caller owns the final squash commit.
6. Do not leave conflict markers in any file.
7. If a conflict is unsafe or unclear, leave it unresolved so the caller can
   abort and roll back honestly.
...
The caller will roll back the integration branch when resolution is exhausted,
so an honest unresolved conflict is safer than a speculative broken merge.
