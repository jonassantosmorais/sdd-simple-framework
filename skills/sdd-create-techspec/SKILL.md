...
Design for the complete PRD scope in this single TechSpec. Agents run long and `sdd-create-tasks` decomposes the work later, so design size is never a reason to trim scope or stage the design into phases.
...
6. Hand off.
   ...
   - Point to `sdd-create-tasks` as the next step.
