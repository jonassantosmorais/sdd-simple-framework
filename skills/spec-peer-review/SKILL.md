...
description: >-
  Independent peer review of a spec — TechSpec, design doc, RFC, or detailed PRD — run via a
  fresh subagent, producing one scoped Markdown findings artifact for user-directed
...
trigger: explicit
argument-hint: "[spec-path] [--context p1,p2] [--out dir] [--model <model>]"
---

# Spec Peer Review

An authoring model drafts a spec; a second, independent reviewer pressure-tests it. Run this
**peer review** only when the user has approved the current draft and explicitly asks for a
round — each round is one user-initiated pass, never auto-run, auto-incorporated, or
auto-looped.

**The runtime's native subagent facility is the review engine**: each round is one dispatch of a
fresh, isolated agent (e.g. the `Agent` tool in Claude Code) with no memory of the authoring
session. The **findings file** the reviewer writes is the source of truth; its returned summary
...
- `--model <name>` (optional, default `opus`): the model to run the reviewer subagent with,
  when the runtime's subagent facility exposes a model override (e.g. the `Agent` tool's
  `model` parameter). When the runtime has no such override, ignore this input.
...
Confirm the runtime exposes a native subagent dispatch mechanism (e.g. the `Agent` tool in
Claude Code). If none exists, abort with a one-line message stating that this skill needs an
isolated subagent to preserve reviewer independence — do not simulate the review by reasoning
about the spec yourself in the current session.
...
2. Dispatch the runtime's native subagent facility (e.g. the `Agent` tool, `run_in_background:
   false`) with the full assembled prompt from `<out>/peer-review-prompt-roundN.md` as its task,
   and `model` set to the resolved `--model` when the facility supports a model override. The
   subagent must be a fresh dispatch with no access to this session's authoring history — it
   reads only the spec, `{context_paths}`, and `{project_rules}` named in the prompt.
...
4. A dispatch error or refusal → fail loudly; do not retry silently, and do not substitute
   reasoning about the spec yourself in its place.
5. The subagent's returned summary is operational evidence only — do not treat it as the
   findings; the findings file is the source of truth.
6. Require the findings file to exist after the subagent returns; missing = invalid round even
   when the subagent reported success.
...
- Spend independent review effort once per round: one subagent dispatch, rerun only when the
  round is invalid and the user asks.

## Error Handling

- **Model misconfiguration (subagent reports the requested model is unavailable):** surface the
  configured model, verify with the user, and record the failure in the round artifacts. Do not
  substitute a model on your own.
- **Missing, malformed, or placeholder findings:** treat as an invalid round — write
  ...
  the validated findings file, never from the subagent's summary.
