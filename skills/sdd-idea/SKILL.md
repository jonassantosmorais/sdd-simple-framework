...
5. Debate trade-offs through multi-advisor council.
   ...
   - Use the runtime's native subagent dispatch (e.g. the `Agent` tool with `subagent_type` set to the advisor's name) against the agents installed from this package's `agents/` directory. Do NOT simulate the advisors inline — each advisor is a real, isolated dispatch. The canonical council roster is: `pragmatic-engineer`, `architect-advisor`, `security-advocate`, `product-mind`, `devils-advocate`, `the-thinker`.
   - If the subagent facility is unavailable or the council agents haven't been installed, stop with an actionable error telling the user to copy `agents/*.md` from this package into their runtime's agent directory (see the package `README.md`).
