...
- The standard council roster is installed as subagents under `agents/<name>.md` in this package (copy to `.claude/agents/` for Claude Code, or the equivalent subagent directory for other runtimes).
- A project may override any advisor by defining its own `.claude/agents/<name>.md` with the same name.
- Dispatch advisors by id, never by driver-specific paths:
  ...
  - `devils-advocate`
  - `the-thinker`
- If the runtime's subagent dispatch (e.g. the `Agent` tool) is unavailable or any selected advisor cannot be resolved, stop and tell the user to copy `agents/*.md` from this package into their runtime's agent directory.
