...
description: >-
  ...
  mismatches. Use when validating completed spec-workflow tasks, AI-authored PRs, or
...
2. **In spec-workflow mode**, write `memory/qa-execution.md` with the the task-execution loop canonical sections (`Objective Snapshot`, `Important Decisions`, `Learnings`, `Files / Surfaces`, `Errors / Corrections`, `Ready for Next Run`) **before** flipping any `task_NN.md` frontmatter (memory-precedes-status invariant).
...
1. Write the report to `<audit-output-path>/audit/audit-report.md` using `assets/audit-report-template.md`, with all mandatory sections:
   ...
   - **TASK IMPLEMENTATION AUDIT** — spec-workflow slug, plan sources, matrix totals, per-task verdicts, reopened/fixed/blocked tasks, links to bugs.
...
2. In a spec-workflow slug, a final PASS feeds the task-execution loop's `verify.last_status=PASS` precondition for Phase E — leave `update-state.py` to the task-execution loop.
