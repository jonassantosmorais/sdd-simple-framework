...
3. **Record classifications in `memory/qa-execution.md` → `Errors / Corrections` section.** This write happens **before** any frontmatter `status:` flip (memory-precedes-status invariant from `the task-execution loop`).
...
## Why this matters in spec-workflow mode

`the task-execution loop` is built around the premise that an agent implements a task and self-reports completion via `state.yaml` and `task_NN.md` frontmatter. `agent-output-audit` is the independent evaluator in that loop. If the auditor accepts the implementer's framing, the loop has no real verification — only ceremonial verification — and `task_NN.md status: completed` becomes a coordination signal, not a quality signal.
